...
const expectedIds = [
  ...Array.from({ length: 38 }, (_, index) => `ST-E2E-${String(index + 1).padStart(2, "0")}`),
  ...Array.from({ length: 8 }, (_, index) => `ST-NF-${String(index + 1).padStart(2, "0")}`),
];
const actualIds = cases.map((item) => item.id);
if (cases.length !== 46 || expectedIds.some((id) => !actualIds.includes(id)) || new Set(actualIds).size !== 46) {
  throw new Error(`Matriz inválida: se esperaban 46 identificadores únicos y se encontraron ${cases.length}.`);
}
