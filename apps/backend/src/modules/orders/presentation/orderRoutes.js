...
const createOrderSchema = z.object({
  items: z.array(z.union([
    z.object({ productId: z.uuid(), quantity: z.coerce.number().int().min(1).max(99) }).strict(),
    z.object({ promotionId: z.uuid(), quantity: z.coerce.number().int().min(1).max(99) }).strict(),
  ])).min(1).max(50),
