...
export default function OrdersAdminPage() {
  ...
  let tableContent;
  if (loading) {
    tableContent = <div className="admin-table-empty">Cargando pedidos...</div>;
  } else if (result.data.length === 0) {
    tableContent = <div className="admin-table-empty">No hay pedidos para este filtro.</div>;
  } else {
    tableContent = <div className="order-table-wrap"><table className="admin-table order-table"><thead><tr><th>Pedido</th><th>Cliente</th><th>Fecha</th><th>Pago</th><th>Total</th><th>Estado</th><th aria-label="Acciones" /></tr></thead><tbody>{result.data.map((order) => { const state = statusInfo[order.status] || { label: order.status, className: "" }; return <tr key={order.id}><td><strong>#{order.id.slice(0, 8).toUpperCase()}</strong><small>{order.items.reduce((sum, item) => sum + item.quantity, 0)} unidades</small></td><td><strong>{order.customerName}</strong><small>{order.customerPhone || order.customerEmail}</small></td><td>{dateTime(order.createdAt)}</td><td><span className={`payment-state ${order.payment?.status === "APPROVED" ? "approved" : ""}`}><FaCreditCard /> {order.payment?.status === "APPROVED" ? "Aprobado" : "Pendiente"}</span></td><td><strong>{money(order.total)}</strong></td><td><span className={`order-status ${state.className}`}>{state.label}</span></td><td><button type="button" className="order-open" onClick={() => setSelected(order)}>Ver <FaArrowRight /></button></td></tr>; })}</tbody></table></div>;
  }
  return <AdminLayout eyebrow="Operación" title="Pedidos" description="Revisa pagos confirmados y organiza la preparación de cada pedido." actions={<button type="button" className="admin-outline-action" onClick={() => load()}><FaRotate /> Actualizar</button>}>
    {error && <div className="admin-alert" role="alert">{error}</div>}
    <section className="order-stats">
      <button type="button" className={!status ? "active" : ""} onClick={() => chooseStatus("")}><span><FaReceipt /></span><div><strong>{result.pagination?.total || 0}</strong><small>Resultados</small></div></button>
      <button type="button" className={status === "CONFIRMED" ? "active" : ""} onClick={() => chooseStatus("CONFIRMED")}><span><FaClock /></span><div><strong>{summary.confirmed || 0}</strong><small>Por preparar</small></div></button>
      <button type="button" className={status === "PREPARING" ? "active" : ""} onClick={() => chooseStatus("PREPARING")}><span><FaFireBurner /></span><div><strong>{summary.preparing || 0}</strong><small>En preparación</small></div></button>
      <button type="button" className={status === "READY" ? "active" : ""} onClick={() => chooseStatus("READY")}><span><FaCircleCheck /></span><div><strong>{summary.ready || 0}</strong><small>Listos</small></div></button>
      <button type="button" className={status === "OUT_FOR_DELIVERY" ? "active" : ""} onClick={() => chooseStatus("OUT_FOR_DELIVERY")}><span><FaArrowRight /></span><div><strong>{summary.outForDelivery || 0}</strong><small>En camino</small></div></button>
    </section>

    <section className="admin-panel-card order-panel">
      <div className="admin-panel-card__toolbar"><div><h2>Cola de pedidos</h2><p>Actualización automática cada 10 segundos{lastUpdated ? ` · Última: ${lastUpdated.toLocaleTimeString("es-PE", { hour: "2-digit", minute: "2-digit", second: "2-digit" })}` : ""}.</p></div><form className="order-search" onSubmit={applySearch}><input value={searchInput} onChange={(event) => setSearchInput(event.target.value)} placeholder="Cliente, correo, teléfono o código" aria-label="Buscar pedidos" /><button type="submit" aria-label="Buscar" title="Buscar"><FaMagnifyingGlass /></button></form></div>
      {tableContent}
      {result.pagination.pages > 1 && <div className="order-pagination"><button type="button" disabled={page <= 1} onClick={() => setPage((value) => value - 1)}>Anterior</button><span>Página {page} de {result.pagination.pages}</span><button type="button" disabled={page >= result.pagination.pages} onClick={() => setPage((value) => value + 1)}>Siguiente</button></div>}
    </section>

    {selected && <div className="order-detail-backdrop"><aside className="order-detail" aria-label="Detalle del pedido"><header><div><span>Pedido</span><h2>#{selected.id.slice(0, 8).toUpperCase()}</h2><p>{dateTime(selected.createdAt)}</p></div><button type="button" onClick={() => setSelected(null)} aria-label="Cerrar"><FaXmark /></button></header>
      ...
      <footer>{nextAction[selected.status] ? <button type="button" className="order-next-action" disabled={updating} onClick={updateStatus}>{(() => { const Icon = nextAction[selected.status].icon; return <Icon />; })()} {updating ? "Actualizando..." : nextAction[selected.status].label}</button> : <p>Este pedido no requiere una acción operativa.</p>}</footer>
    </aside></div>}
  </AdminLayout>;
}
