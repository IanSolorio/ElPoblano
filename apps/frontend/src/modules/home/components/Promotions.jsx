...
export default function Promotions() {
  ...
  const renderCard = (promo, index) => {
    ...
    return <article className="home-promo-card" key={`${promo.id}-${index}`} onFocus={() => !duplicate && setCurrent(originalIndex)} aria-hidden={duplicate ? "true" : undefined}><div className="home-promo-card__image"><img src={promo.image} alt={duplicate ? "" : promo.product} /><span>{promo.kind === "BUNDLE" ? "Combo" : "Oferta activa"}</span></div><div className="home-promo-card__body"><p>{promo.title}</p><h3>{promo.product}</h3>{promo.description && <small>{promo.description}</small>}<div className="home-promo-card__purchase"><div><del>S/ {promo.originalPrice.toFixed(2)}</del><strong>S/ {promo.price.toFixed(2)}</strong></div><button type="button" tabIndex={duplicate ? -1 : 0} onClick={() => addPromotionToCart(promo)}><FaCartPlus /> Agregar</button></div></div></article>;
  };

  let promotionContent;
  if (loading) {
    promotionContent = <div className="home-promotions__grid" aria-label="Cargando promociones">{[1, 2, 3].map((item) => <div className="home-promo-skeleton" key={item} />)}</div>;
  } else if (promotions.length === 0) {
    promotionContent = <div className="home-promotions__empty"><span><FaTag /></span><div><h3>Muy pronto habrá nuevas promociones</h3><p>Mientras tanto, conoce todos los sabores disponibles en nuestra carta.</p></div><Link className="home-button home-button--dark" to="/productos">Ver productos</Link></div>;
  } else {
    const carouselItems = Array.from({ length: Math.max(promotions.length * 2, 8) }, (_, index) => promotions[index % promotions.length]);
    promotionContent = <><div className="home-promotions__viewport"><div className="home-promotions__track" ref={carousel} onScroll={syncCurrentSlide} onPointerDown={startDrag} onPointerMove={moveDrag} onPointerUp={stopDrag} onPointerCancel={stopDrag} onClickCapture={preventDraggedClick}>{carouselItems.map((promotion, index) => renderCard(promotion, index))}</div></div>{promotions.length > 1 && <div className="home-carousel-dots" aria-label="Seleccionar promoción">{promotions.map((promo, index) => <button type="button" className={index === current ? "active" : ""} onClick={() => goTo(index)} aria-label={`Ir a ${promo.product}`} key={`${promo.id}-${index}`} />)}</div>}</>;
  }
